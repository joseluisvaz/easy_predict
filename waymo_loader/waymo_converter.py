...
def _generate_records_from_files(files: T.List[str], max_agents: int = 8) -> T.Generator[T.Dict[str, np.ndarray], None, None]:
    ...
    for payload in dataset.as_numpy_iterator():
        ...
        for key, value in decoded_example.items():
            ...
            if cropped_value.shape[0] < max_agents:
                padding = ((0, max_agents - cropped_value.shape[0]),) + ((0, 0),) * (cropped_value.ndim - 1)
...
def _create_h5_datasets(file: h5py.File, new_data: np.ndarray, max_agents: int=8) -> None:
    ...
    file.create_dataset("merged_features", data=tensor, chunks=True, maxshape=maxshape, compression="gzip")
