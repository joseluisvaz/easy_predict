...
def mask_only_target_agents_and_sdc(
    ...
    target_agents = np.logical_or(
        batch["tracks_to_predict"], batch["is_sdc"]
    )  # [N_AGENTS,]
    ...
    batch["gt_states_avails"] = force_pad_batch_size(
        batch["gt_states_avails"], MAX_N_AGENTS
    )
    ...
    batch["tracks_to_predict"] = force_pad_batch_size(
        batch["tracks_to_predict"], MAX_N_AGENTS
    )
    return batch
...
def _get_scenario_from_h5_file_using_idx(
    file: T.Mapping[str, np.ndarray], idx: int
) -> T.Dict[str, np.ndarray]:
    return {
        "scenario_id": np.array(file["scenario_id"][idx]).astype(np.int64),
        "gt_states": np.array(file["gt_states"][idx]).astype(
            np.float32
        ),  # [N_AGENTS, TIME, FEATS]
        ...
        "roadgraph_features_mask": np.array(
            file["roadgraph_features_mask"][idx]
        ).astype(np.bool_),  # [N_POLYLINE, N_POINTS]
        "roadgraph_features_types": np.array(
            file["roadgraph_features_types"][idx]
        ).astype(np.int64),  # [N_POLYLINE,]
        ...
        "tl_avails": np.array(file["tl_avails"][idx]).astype(
            np.bool_
        ),  # [N_TRAFFIC_LIGHTS, TIME,]
    }
